...
class PromptManager {
  ...
  renderPrompts() {
    ...
    container.innerHTML = `
      <div class="cg-prompt-actions">
        ...
        <button class="cg-btn cg-btn-secondary cg-btn-small" id="cg-export-prompts">Export</button>
        <button class="cg-btn cg-btn-secondary cg-btn-small" id="cg-backup-all">Backup All</button>
      </div>
  ...
  setupPromptEventListeners() {
    ...
    container.addEventListener('click', async (e) => {
      ...
      } else if (target.id === 'cg-import-prompts') {
        this.showImportDialog();
      } else if (target.id === 'cg-export-prompts') {
        this.exportPrompts();
      } else if (target.id === 'cg-backup-all') {
        this.backupAllData();
  ...
  showImportDialog() {
    ...
    const modal = document.createElement('div');
    modal.className = 'cg-modal cg-import-modal';
    modal.innerHTML = `
      <div class="cg-modal-backdrop"></div>
      <div class="cg-modal-content">
        <div class="cg-modal-header">
          <h3 class="cg-modal-title">Import Data</h3>
          <button class="cg-modal-close" data-action="close-import-modal">×</button>
        </div>
        <div class="cg-modal-body">
          <div class="cg-import-options">
            <div class="cg-import-section">
              <h4>Import Prompts</h4>
              <input type="file" id="cg-import-prompts-file" accept=".json" />
              <button class="cg-btn cg-btn-small" id="cg-import-prompts-btn">Import Prompts</button>
            </div>
            <div class="cg-import-section">
              <h4>Import Complete Backup</h4>
              <input type="file" id="cg-import-backup-file" accept=".json" />
              <button class="cg-btn cg-btn-small" id="cg-import-backup-btn">Import Backup</button>
            </div>
          </div>
          <div class="cg-form-actions">
            <button type="button" class="cg-btn cg-btn-secondary" data-action="close-import-modal">Cancel</button>
          </div>
        </div>
      </div>
    `;
  ...
  setupImportHandlers(modal) {
    modal.addEventListener('click', (e) => {
      if (e.target.dataset.action === 'close-import-modal' || e.target.classList.contains('cg-modal-backdrop')) {
        this.closeModal();
      }
    });

    modal.querySelector('#cg-import-prompts-btn').addEventListener('click', () => {
      const fileInput = modal.querySelector('#cg-import-prompts-file');
      const file = fileInput.files[0];
      if (file) {
        this.importPromptsFromFile(file);
      } else {
        this.parent.showToast('Please select a file to import');
      }
    });

    modal.querySelector('#cg-import-backup-btn').addEventListener('click', () => {
      const fileInput = modal.querySelector('#cg-import-backup-file');
      const file = fileInput.files[0];
      if (file) {
        this.importBackupFromFile(file);
      } else {
        this.parent.showToast('Please select a backup file to import');
      }
    });
  }
  ...
  async importPromptsFromFile(file) {
    try {
      ...
      this.renderPrompts();
      this.closeModal();
      this.parent.showToast(`Imported ${importedPrompts.length} prompts successfully`);
    } catch (error) {
      console.error('Failed to import prompts:', error);
      this.parent.showToast('Failed to import prompts. Please check file format.');
    }
  }
  ...
  async importBackupFromFile(file) {
    try {
      ...
      if (backup.prompts) {
        this.prompts = backup.prompts;
        await this.savePrompts();
      }
      
      if (backup.folders && this.parent.folderManager) {
        this.parent.folderManager.folders = backup.folders;
        await this.parent.folderManager.saveFolders();
      }
      
      if (backup.conversations && this.parent.folderManager) {
        this.parent.folderManager.conversations = backup.conversations;
        await this.parent.folderManager.saveConversations();
      }
      
      this.renderPrompts();
      if (this.parent.folderManager) {
        this.parent.folderManager.renderFolders();
      }
      
      this.closeModal();
      this.parent.showToast('Backup imported successfully');
    } catch (error) {
      console.error('Failed to import backup:', error);
      this.parent.showToast('Failed to import backup. Please check file format.');
    }
  }

  exportPrompts() {
    try {
      const exportData = JSON.stringify(this.prompts, null, 2);
      ...
      this.parent.showToast('Prompts exported successfully');
    } catch (error) {
      console.error('Failed to export prompts:', error);
      this.parent.showToast('Failed to export prompts');
    }
  }
  ...
  async backupAllData() {
    try {
      const backup = {
        ...
        folders: this.parent.folderManager ? this.parent.folderManager.folders : [],
        conversations: this.parent.folderManager ? this.parent.folderManager.conversations : [],
        settings: {
          sidebarOpen: localStorage.getItem('cg-sidebar-open'),
          sidebarInteracted: localStorage.getItem('cg-sidebar-interacted'),
          togglePosition: localStorage.getItem('cg-toggle-position')
        }
      };
      ...
      this.parent.showToast('Complete backup exported successfully');
    } catch (error) {
      console.error('Failed to create backup:', error);
      this.parent.showToast('Failed to create backup');
    }
  }
